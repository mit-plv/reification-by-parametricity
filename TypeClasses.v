...
Section with_var.
  ...
  Program Global Instance reify_NatMul `{reify_of x rx, reify_of y ry}
    : reify_of (x * y) (rx * ry).
  Program Global Instance reify_LetIn `{reify_of x rx}
         `{forall y ry, reify_of y (Var ry) -> reify_of (f y) (rf ry)}
    : reify_of (dlet y := x in f y) (elet ry := rx in rf ry).
  Program Global Instance reify_S `{reify_of x rx}
    : reify_of (S x) (NatS rx).
  Program Global Instance reify_O
    : reify_of O NatO.
End with_var.
